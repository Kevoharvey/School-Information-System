...
Here’s a full breakdown organized by feature area.
...
Dynamically filters by grade level, status, and a search term across name/email/ID. Empty strings are used as “no filter” — a neat trick to avoid building dynamic SQL. 🧠
...
Add/Edit/Delete follow the same Users + Employee + Instructor three-table pattern as students.
...
ensure_department() and ensure_classroom() are upsert-style helpers — create the row only if it doesn’t exist yet.
...
Smart trick: ORDER BY a.Due_Date IS NULL pushes undated assignments to the bottom without needing a CASE. The LEFT JOIN Submission with the student’s ID attaches their own submission (if any) in one shot.
Teacher/Admin view
...
Shows submission count vs. total students — basically a real-time completion rate.
Submit Assignment
...
-- Syncs grade into Studies table
...
This is the upsert logic that keeps the grade book (Studies) in sync with assignment scores. ✅
...
Shows which teachers are most active in creating work. 👆
Monthly Stats
...
Monthly assignment creation trend — good for spotting seasonal spikes (exam season anyone? 😅).
...
ORDER BY FIELD(se.Day_Of_Week,'Monday','Tuesday',...), se.Start_Time


FIELD() is the MySQL way to sort by a custom order (Mon→Fri) rather than alphabetically.
Add Schedule Entry
...
Loops through all students and inserts one notification per student. Works, but would hit performance at scale — a broadcast column would be more efficient for large schools. 💡

✅ Attendance

SELECT Subject_ID, Subject_Name FROM Subject ORDER BY Subject_Name

SELECT st.Student_ID, st.Fname, st.Lname, st.Student_Email FROM Student
...
Loads the subject list, all students, and any existing records for that date.
Save Attendance (upsert loop)
...
-- Uses REGISTRATION_UNION_QUERY which is:
SELECT ... FROM Student_Registration
UNION ALL
SELECT ... FROM Teacher_Registration


This UNION ALL normalizes two completely different registration tables into one consistent shape so they can be filtered, sorted, and paginated together. Very clean design. 👏
Approve → Create Account
Calls create_student_from_registration() or create_teacher_from_registration() which run the same Users + Student/Employee + Instructor insert chains described above.
...
Stamps the registration as Approved or Rejected after processing.

🤖 AI Assistant (Gemini / Fallback)

-- Dynamically generated by AI or fallback, e.g.:
...
GROUP BY st.Student_ID ORDER BY average_grade DESC LIMIT 10


The AI takes a natural-language question, generates a SELECT query, validates it (no INSERT/DROP/etc.), and runs it. The fallback patterns cover common questions like “top students” or “who failed math” when the Gemini API is unavailable.
